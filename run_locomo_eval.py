...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build memory on a slice of LoCoMo, evaluate on the rest."
    )
    parser.add_argument(
        "--locomo-path",
        default=None,
    ...
    parser.add_argument(
        "--categories",
        default=",".join(str(c) for c in DEFAULT_EVAL_CATEGORIES),
    ...
    parser.add_argument(
        "--max-chars-per-call",
        type=int,
        default=DEFAULT_MAX_CHARS_PER_CALL,
    ...
    parser.add_argument(
        "--output", default=None, help="Path to dump full per-question results as JSON."
    )
...
def build_offline_memory(
    ...
    *,
    ...
    for raw_conv in bootstrap_raw:
        ...
        print(
            f"[bootstrap] {conv['sample_id']}: distilling {len(conv['turns'])} turn(s) into a CTC graph..."
        )
        ...
        print(
            f"[bootstrap] {conv['sample_id']}: collecting {len(seed_questions)} unguided trajectory(ies)..."
        )
    ...
    print(
        f"[bootstrap] built experience bank: {len(bank.planning_bank)} Planning + "
...
def main() -> None:
    ...
    bootstrap_raw, eval_raw = split_bootstrap_eval(
        raw_conversations, bootstrap_fraction=args.bootstrap_fraction
    )
    if args.max_eval_conversations is not None:
        eval_raw = eval_raw[: args.max_eval_conversations]
    print(
        f"[setup] {len(raw_conversations)} conversation(s) total: "
    ...
    bank = build_offline_memory(
        ...
        categories=categories,
        bootstrap_questions_per_conv=args.bootstrap_questions_per_conv,
    ...
    for raw_conv in eval_raw:
        ...
        graph = build_ctc_graph_from_dialogue(
            conv["turns"], llm, max_chars_per_call=args.max_chars_per_call
        )
        if args.save_graphs_dir:
        ...
        conv_results = evaluate_conversation(
            ...
            bank,
            max_iterations=args.max_iterations,
            run_judge=not args.skip_llm_judge,
        )
        all_results.extend(conv_results)

        conv_f1 = sum(r["f1"] for r in conv_results) / len(conv_results) if conv_results else 0.0
        print(
            f"[eval] {conv['sample_id']}: done ({len(conv_results)} question(s), avg F1 {conv_f1 * 100:.2f}%)"
        )
